extension Usecase {
    protocol AuthUsecase {
        func verifyTokenAndStartSession(idToken: String, accessToken: String, refreshToken: String)
            async -> Result<Bool, UsecaseError>
    }
}
...
extension Usecase {
    class AuthUsecaseImpl: AuthUsecase {
        ...
        func verifyTokenAndStartSession(idToken: String, accessToken: String, refreshToken: String)
            async -> Result<Bool, Usecase.UsecaseError>
        {
            let result = await self.authRepo.verifyTokenAndStartSession(
            ...
            case .failure(let err):
                return .failure(
                    .init(
                        message: "failed authRepo.verifyTokenAndStartSession", innerError: err,
                        kind: .internalError))
            }
        }
    }
