extension Usecase {
    protocol ArticleUsecase {
        ...
        func find(offset: Int, limit: Int, searchTitleText: String?) async -> Result<[DomainModel.ArticleSummary], UsecaseError>
    }
}
...
public extension Usecase {
    class ArticleUsecaseImpl: ArticleUsecase {
        ...
        func find(offset: Int, limit: Int, searchTitleText: String?) async -> Result<[DomainModel.ArticleSummary], Usecase.UsecaseError> {
            let articles = await self.articleRepo.find(offset: offset, limit: limit, searchTitleText: searchTitleText)
            switch articles {
            case .success(let articles):
                return .success(articles)
            case .failure(let err):
                return .failure(.init(message: "failed find articles", innerError: err, kind: .internalError))
            }
        }
    }
}
    ...
    struct ArticleUsecaseMock: ArticleUsecase {
        ...
        func get(id: String) async -> Result<DomainModel.Article, Usecase.UsecaseError> {
            guard let result = self.getResult else {
                return .failure(.init(message: "unspecfied", innerError: nil, kind: .internalError))
            }
            return result
        }
