            ...
            async -> Result<Bool, DomainRepo.RepoError>
        {
            guard
                var components = URLComponents(
                    string: "\(self.baseUrl)\(self.verifyTokenAndStartSessionApiPath)")
            else {
                return .failure(
                    .init(
                        message:
                            "failed make compoents. baseUrl: \(baseUrl), path: \(verifyTokenAndStartSessionApiPath)",
                        innerError: nil, kind: .internalError))
            }
            ...
            guard let url = components.url else {
                return .failure(
                    .init(message: "invalid url", innerError: nil, kind: .internalError))
            }
            ...
            if resp.statusCode != 200 {
                return .failure(
                    .init(
                        message:
                            "failed verify token and start session. statusCode: \(resp.statusCode)",
                        innerError: nil, kind: .internalError))
            }
            return .success(true)
        }
