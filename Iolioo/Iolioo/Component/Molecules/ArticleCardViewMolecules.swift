...
struct ArticleCardViewMolecules_Previews: PreviewProvider {
    static var previews: some View {
        ...
        let tags: [DomainModel.ArticleTag] = [
            DomainModel.ArticleTag(id: "tag_id", name: "tag_name"),
            DomainModel.ArticleTag(id: "tag_id", name: "tag_name"),
        ]

        ArticleCardViewMolecules(
            summary: DomainModel.ArticleSummary.init(
                id: "id", title: "📓 English Diary: Yesterday's Happen", tags: tags,
                createdAt: Date.now, updatedAt: Date.now)
        ).background().previewLayout(.fixed(width: 400, height: 60))
    }
}
