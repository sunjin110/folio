...
func (c *GolioAPIController) ArticlesTagsTagIdPut(w http.ResponseWriter, r *http.Request) {
	...
	articlesTagsTagIdPutRequestParam := ArticlesTagsTagIdPutRequest{}
	d := json.NewDecoder(r.Body)
	d.DisallowUnknownFields()
	if err := d.Decode(&articlesTagsTagIdPutRequestParam); err != nil && !errors.Is(err, io.EOF) {
		c.errorHandler(w, r, &ParsingError{Err: err}, nil)
		return
	}
	if err := AssertArticlesTagsTagIdPutRequestRequired(articlesTagsTagIdPutRequestParam); err != nil {
		c.errorHandler(w, r, err, nil)
		return
	}
	if err := AssertArticlesTagsTagIdPutRequestConstraints(articlesTagsTagIdPutRequestParam); err != nil {
		c.errorHandler(w, r, err, nil)
		return
	}
	result, err := c.service.ArticlesTagsTagIdPut(r.Context(), tagIdParam, articlesTagsTagIdPutRequestParam)
	// If an error occurred, encode the error with the status code
	if err != nil {
		c.errorHandler(w, r, err, &result)
		return
