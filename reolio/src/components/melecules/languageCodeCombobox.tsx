...
export function LanguageCodeCombobox(props: LanguageCodeComboboxProps) {
    ...
    const {languageCode, setLanguageCode, emptyText, className, languages} = props;
    ...
    return <div className={className}>
        ...
        <PopoverContent className="w-[200px] p-0">
            <Command className="bg-black text-white">
                ...
                <CommandList>
                    <CommandEmpty className="text-white">No language found.</CommandEmpty>
                    <CommandGroup>
                        {languages.map((language) => (
                            <CommandItem
                                ...
                                onSelect={(currentCode: string) => {
                                    setLanguageCode(currentCode === languageCode ? "" : currentCode);
                                    setOpen(false);
                                }}
                                className="hover:bg-zinc-900"
                            >
                                <Check className={cn("mr-2 h-4 w-4", languageCode === language.code ? "opacity-100" : "opacity-0")} />
                                {language.name}
                            </CommandItem>
                        ))}
                    </CommandGroup>
