...
export function NewTranslationRepository(
  golioApi: GolioApi,
...
class translation implements TranslationRepository {
  ...
  async TranslationText(
    text: string,
    sourceLanguageCodeStr: string,
    targetLanguageCodeStr: string,
  ): Promise<string> {
    const sourceLanguageCode = translation.toLanguageCode(
      sourceLanguageCodeStr,
    );
    if (!sourceLanguageCode) {
      throw new Error(
        `bad request sourceLanguageCode: ${sourceLanguageCodeStr}`,
      );
    }

    const targetLanguageCode = translation.toLanguageCode(
      targetLanguageCodeStr,
    );
    if (!targetLanguageCode) {
      throw new Error(
        `bad request targetLanguageCode: ${targetLanguageCodeStr}`,
      );
    }
